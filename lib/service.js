  ...
  .then(function(data){
    ...
    .then(function(info){
      ...
      if(data.destEnv.archiveOnPromote){
        //DocScores-v1.2.1-20160405.144261.war
    ...
    .then(function(info){
      ...
      return updateInfoJson(data.destEnv, info.build, ver);
    });
...
function updateInfoJson(env, build, version){
...
function _downloadRemoteBuild(verbose){
  ...
  return new Promise(function(resolve, reject) {
    ...
    var params = {
      'auth': {
        'user': rb.credentials.user,
        'pass': rb.credentials.pass,
        'sendImmediately': false
      }
    };

    if(verbose){
      console.log("Downloading remote build from " + rb.url);
    }
    ...
    .pipe(fs.createWriteStream(rb.localBuildFile));
  });
