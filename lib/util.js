...
var s3 = new BBPromise.promisifyAll(new AWS.S3(), {suffix: 'UsingPromise'});
...
function copyLocalFileToS3UsingPromise(localFilePath, bucket, s3FileName, verbose){
  ...
  return new Promise(function(resolve, reject) {
    ...
    if(verbose){
      console.log('Uploading ' + localFilePath + ' to S3 ' + bucket + ':' + s3FileName);
    }
...
function copyS3FileToS3UsingPromise(srcPath, destPath, bucket, verbose){
  ...
  .then(function(data){
    if(verbose){
      console.log('Successful S3 copy of ' + srcPath + ' to ' + destPath + ' in bucket ' + bucket);
    }
...
function putJsonObjectUsingPromise(jsonObj, bucket, s3FileName, verbose){
  ...
  .then(function(data){
    if(verbose){
      console.log('Successful upload of json obj to S3 ' + bucket + ':' + s3FileName);
    }
